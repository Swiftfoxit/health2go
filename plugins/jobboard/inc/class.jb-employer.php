...
class JobBoard_Employer
{
    ...
    function __construct()
    {
        ...
        add_filter('jobboard_dashboard_navigation_jobs_title', array($this, 'get_navigation_jobs_title'));
    }
    ...
    function get_applications($post_id, $page = 0, $search = '')
    {
        ...
        update_post_meta($post_id, '_application_ids', array());
    ...
    function get_navigation_jobs_title($title)
    {
        global $jobboard, $wpdb;
        ...
        $applications = $wpdb->get_col($wpdb->prepare(
            "SELECT m.meta_value
                  ...
                  AND p.post_status = %s
                  AND p.post_author = %d"
            , '_application_ids', 'publish', get_current_user_id())
        );

        $count = 0;
        if ($applications) {
            foreach ($applications as $app) {
                $count += count(maybe_unserialize($app));
            }
        }

        ob_start();

        jb_get_template('global/count.php', array('count' => $count));
    ...
    function ajax_delete_job()
    {
        ...
        exit(sprintf(esc_html__("Successfully Deleted for '%s'", JB_TEXT_DOMAIN), $post->post_title));
    }
