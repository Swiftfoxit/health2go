...
if(!class_exists('JobBoard_Emails')) {
    ...
    class JobBoard_Emails
    {
        ...
        function send()
        {
            ...
            remove_action('phpmailer_init', array($this, 'smtp_config'));
        }

        function smtp_config($phpmailer){
            if('php' === jb_get_option('email-function', 'php')){
                return;
            }

            $user       = jb_get_option('email-smtp-login');
            $host       = jb_get_option('email-smtp-host');
            $auth       = jb_get_option('email-smtp-auth', true);
            $port       = jb_get_option('email-smtp-port');
            $username   = !empty($user['username']) ? $user['username'] : '';
            $password   = !empty($user['password']) ? $user['password'] : '';
            $secure     = jb_get_option('email-smtp-secure');
            ...
            $phpmailer->isSMTP();
            $phpmailer->Host        = $host;
            $phpmailer->SMTPAuth    = (boolean)$auth;
            $phpmailer->Port        = $port;
        ...
        function candidate_applied($post, $employer, $candidate)
        {
            ...
            $this->reply    = $employer->user_email;
        ...
        function employer_applied($post, $employer, $candidate){
            ...
            $this->reply        = $candidate->user_email;
