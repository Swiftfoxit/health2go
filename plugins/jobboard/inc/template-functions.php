...
function jb_template_breadcrumb($args = array())
{
    $args = wp_parse_args($args, apply_filters('jb/template/breadcrumb/args', array(
        ...
        'before' => '',
        'after' => '',
        'home' => _x('Home', 'breadcrumb', JB_TEXT_DOMAIN)
    )));
...
function jb_template_catalog_orderby()
{
    ...
    $catalog_orderby_options = apply_filters('jobboard_catalog_orderby_args', array(
        'name' => esc_html__('Sort by name', JB_TEXT_DOMAIN),
        'applied' => esc_html__('Sort by applied', JB_TEXT_DOMAIN),
        'date' => esc_html__('Sort by date posted', JB_TEXT_DOMAIN),
        'salary' => esc_html__('Sort by salary', JB_TEXT_DOMAIN)
    ));

    jb_get_template('archive/actions/orderby.php', array(
...
function jb_template_search_form()
{
    $search = apply_filters('jobboard_search_form_args', array(
        'name' => 's',
        'placeholder' => esc_attr_x('Search Jobs&hellip;', 'placeholder', JB_TEXT_DOMAIN),
        'button' => esc_attr_x('Search', 'submit button', JB_TEXT_DOMAIN),
...
function jb_template_add_job_form()
{
    if (!is_user_logged_in()) {
        jb_get_template('modal/modal-start.php', array('modal' => 'jobboard-login-add-job'));
...
function jb_template_job_loop_summary_title()
{
    ...
    if ($featured) {
        $status = array('id' => 'featured', 'name' => esc_html__('Featured', JB_TEXT_DOMAIN));
    }
...
function jb_template_job_loop_specialisms_listing()
{
    jb_get_template('global/specialisms-listing.php');
}
...
function jb_template_job_map()
{
    jb_get_template('single/map.php');
}
...
function jb_template_job_author()
{
    jb_get_template('single/author-info.php');
}
...
function jb_template_user_employer_summary()
{
    $summaries = apply_filters('jobboard_user_employer_summary_args', array(
        jb_employer_get_vacancies() => esc_html__('Vacancies: %s', JB_TEXT_DOMAIN),
        jb_account_get_count() => esc_html__('Current Jobs : %s', JB_TEXT_DOMAIN),
        jb_account_get_date() => esc_html__('Established: %s', JB_TEXT_DOMAIN),
        jb_account_get_email() => array(
            'before' => '<a href="mailto:%s">',
            'title' => esc_html__('Email: %s', JB_TEXT_DOMAIN),
            'after' => '</a>'
        ),
        jb_account_get_website() => array(
            'before' => '<a href="%s">',
            'title' => esc_html__('Visit Website: %s', JB_TEXT_DOMAIN),
            'after' => '</a>'
        )
    ));
...
function jb_template_user_candidate_summary()
{
    $summaries = apply_filters('jobboard_user_candidate_summary_args', array(
        jb_candidate_get_salary() => esc_html__('Minimum Salary: %s', JB_TEXT_DOMAIN),
        jb_account_get_date() => esc_html__('Established: %s', JB_TEXT_DOMAIN),
        jb_account_get_email() => array(
            'before' => '<a href="mailto:%s">',
            'title' => esc_html__('Email: %s', JB_TEXT_DOMAIN),
...
function jb_template_user_candidate_summary_button()
{
    $url = jb_candidate_get_cv_url();
    if (isset($url) && !empty($url) && $url !== '#') {
        jb_get_template('user/cv.php', array('cv' => jb_candidate_get_cv_url()));
    }
}
...
function jb_template_user_video()
{
    global $jobboard_account;
    $video = get_user_meta($jobboard_account->ID, 'user_video', true);
    jb_get_template('user/video.php', array('user_video' => $video));
}
...
function jb_template_login_from($args = array())
{
    ...
    $defaults = array(
        'form_id' => 'form-' . uniqid(),
        'label_username' => esc_html__('Username or Email Address', JB_TEXT_DOMAIN),
...
function jb_template_candidate_applied()
{
    ...
    $columns = apply_filters('jobboard_table_applied_columns', array(
        ...
        'type' => esc_html__('Type', JB_TEXT_DOMAIN),
        'date' => esc_html__('Date Applied', JB_TEXT_DOMAIN),
        'status' => esc_html__('Status', JB_TEXT_DOMAIN),
    ));

    $wp_query = JB()->candidate->get_applied();

    jb_get_template('dashboard/global/table.php', array(
        'jobs' => $wp_query,
        'table' => 'applied',
        'columns' => $columns
    ));
...
function jb_template_candidate_applied_title()
{
    global $post, $jobboard;

    $status = '';
    if (!empty($jobboard->candidate->applied_new) && in_array($post->ID, $jobboard->candidate->applied_new)) {
        $status = array(
            'id' => 'new',
            'name' => esc_html__('New', JB_TEXT_DOMAIN)
        );
        $jobboard->candidate->applied_remove[] = $post->ID;
    }
...
function jb_template_employer_jobs()
{
    ...
    $columns = apply_filters('jobboard_table_jobs_columns', array(
        'title' => esc_html__('Your Jobs', JB_TEXT_DOMAIN),
        'applications' => esc_html__('Applications', JB_TEXT_DOMAIN),
        'status' => esc_html__('Status', JB_TEXT_DOMAIN),
        'actions' => esc_html__('Actions', JB_TEXT_DOMAIN),
    ));
...
function jb_template_employer_jobs_actions()
{
    ...
    $actions = apply_filters('jobboard_template_jobs_actions', array(
        array(
            'id' => 'edit',
            'icon' => 'fa fa-pencil',
            'title' => esc_html__('Edit', JB_TEXT_DOMAIN),
            'attribute' => array(
                'data-url' => $edit
            )
        ),
        array(
            'id' => 'delete',
            'icon' => 'fa fa-times',
            'title' => esc_html__('Delete', JB_TEXT_DOMAIN),
            'attribute' => array(
                'data-id' => $post->ID,
                'data-confirm' => esc_html__('Do you want to delete', JB_TEXT_DOMAIN),
...
{
    $columns = apply_filters('jobboard_table_jobs_columns', array(
        'candidates' => esc_html__('Applications', JB_TEXT_DOMAIN),
        'cv' => esc_html__('Download CV', JB_TEXT_DOMAIN),
        'status' => esc_html__('Status', JB_TEXT_DOMAIN),
        'actions' => esc_html__('Actions', JB_TEXT_DOMAIN),
    ));
...
function jb_build_content_locations_page($contents)
{
    if (is_page(jb_page_id('locations'))) {
        $country = 'all';
        $count = 'all';
        if (!empty($_REQUEST['country'])) {
            $country = $_REQUEST['country'];
        }
        $contents = '[cms_locations layout_type="grid" view_all="hidden" country="' . $country . '" count="' . $count . '" col_xs="1" col_sm="2" col_md="3" col_lg="3"]';
    }
    return $contents;
}
...
function jb_template_employer_application_actions($id)
{
    $actions = apply_filters('jobboard_template_application_actions', array(
        array(
            'id' => 'approve',
            'icon' => 'fa fa-check',
            'title' => esc_html__('Approve', JB_TEXT_DOMAIN),
            'attribute' => array(
                'data-id' => $id,
            )
        ),
        array(
            'id' => 'reject',
            'icon' => 'fa fa-times',
            'title' => esc_html__('Reject', JB_TEXT_DOMAIN),
            'attribute' => array(
                'data-id' => $id,
            )
        )
...
function jb_get_lastest_job($limit = 3)
{
