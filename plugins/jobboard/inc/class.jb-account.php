...
if (! class_exists('JobBoard_Account')) :
    ...
    class JobBoard_Account{
        ...
        function orderby_args($args){
            ...
            return array(
                'ID'            => esc_html__( 'Sort by ID', JB_TEXT_DOMAIN ),
                'display_name'  => esc_html__( 'Sort by name', JB_TEXT_DOMAIN ),
                'registered'    => esc_html__( 'Sort by registered', JB_TEXT_DOMAIN )
            );
        }
        ...
        function search_form_args($args){
            ...
            return array(
                'name'          => 'search',
                'placeholder'   => esc_attr_x( 'Search Name&hellip;', 'placeholder', JB_TEXT_DOMAIN ),
        ...
        function login_error(){
            ...
            switch ($_GET['error']){
                case '1':
                    jb_notice_add( esc_html__( 'Error : Email or password incorrect!', JB_TEXT_DOMAIN ), 'error');
                    break;
            }
        }
        ...
        function user_sections(){
            ...
            if(is_jb_employer($user->ID)){
                ...
                $fields[] = array(
                    ...
                    'subtitle'  => esc_html__( 'Select or upload a avatar image.', JB_TEXT_DOMAIN ),
                );
            }
            ...
            return array(
                'basic' => array(
                    'title'     => esc_html__( 'JobBoard Profile', JB_TEXT_DOMAIN ),
                    'id'        => 'basic',
        ...
        function user_profile_menu(){
            add_submenu_page(
                'profile.php',
                esc_html__( 'Extended Profile',  JB_TEXT_DOMAIN ),
                esc_html__( 'Extended Profile',  JB_TEXT_DOMAIN ),
                'manage_options',
        ...
        function profile_nav($user = null, $active = 'WordPress'){
            ...
            <h2 id="profile-nav" class="nav-tab-wrapper">
                <?php if ( current_user_can( 'edit_user', $user->ID ) ) : ?>
                    <a class="nav-tab<?php echo esc_attr( $wp_active ); ?>" href="<?php echo esc_url($wordpress_url);?>"><?php esc_html_e( 'Profile', JB_TEXT_DOMAIN ); ?></a>
                <?php endif; ?>
                <a class="nav-tab<?php echo esc_attr( $jb_active ); ?>" href="<?php echo esc_url( $community_url );?>"><?php esc_html_e( 'Extended Profile', JB_TEXT_DOMAIN ); ?></a>
            </h2>
            <?php
        }
        ...
        function user_admin(){
            ...
            <div class="wrap" id="community-profile-page">
                <h1>
                    <?php esc_html_e( 'Extended Profile',  JB_TEXT_DOMAIN ); ?>
                    <a href="user-new.php" class="add-new-h2"><?php echo esc_html_x( 'Add New', 'user', JB_TEXT_DOMAIN ); ?></a>
                </h1>
                ...
                <?php else: ?>
                    <p><?php
                        printf(
                            ...
                            __( 'No user found with this ID.', JB_TEXT_DOMAIN ),
                            esc_url( admin_url( 'users.php' ) ),
                            __( 'Go back and try again.', JB_TEXT_DOMAIN )
                        );
                    ?></p>
                <?php endif; ?>
            </div>
