...
if ( ! class_exists( 'ReduxFramework_rc_time' ) ) {
	class ReduxFramework_rc_time {
		...
		function render() {
			...
			echo '<input '. $min . $max . $step . $qtip_title . $qtip_text . 'type="time" id="' . $this->field['id'] . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '" ' . $placeholder . 'value="' . esc_attr( $this->value ) . '" class="regular-text regular-number ' . $this->field['class'] . '"' . $readonly . $autocomplete .' />';
		}
	}
}