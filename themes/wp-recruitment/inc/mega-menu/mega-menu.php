...
add_action( 'wp_nav_menu_item_custom_fields', 'wp_recruitment_add_menu_submenu_type_fields', 10, 4 );

function wp_recruitment_add_menu_submenu_type_fields( $item_id, $item, $depth, $args ) {

    if ( $depth )
        return;

    $title = esc_html__('Submenu Type', 'wp-recruitment'); $key = "menu-item-submenu_type"; $value = $item->submenu_type;
    ...
    ?><p class="description description-wide description_width_100">
        ...
        <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
            <select id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>">
                <option value="standard" <?php echo ( $value == 'standard' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Standard Dropdown', 'wp-recruitment' ); ?></option>
                <option value="columns2" <?php echo ( $value == 'columns2' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '2 columns dropdown', 'wp-recruitment' ); ?></option>
                <option value="columns3" <?php echo ( $value == 'columns3' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '3 columns dropdown', 'wp-recruitment' ); ?>
                </option>
                <option value="columns4" <?php echo ( $value == 'columns4' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '4 columns dropdown', 'wp-recruitment' ); ?></option>
            </select>
        </label>
    </p>
    <?php
    $title = esc_html__('Sub menu column width ( Example: 200)', 'wp-recruitment');
    ...
    <?php
    $title = esc_html__('Side of Dropdown Elements', 'wp-recruitment');
    ...
    <p class="description description-wide description_width_100">
        ...
        <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
            <select id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>">
                <option value="autodrop_submenu" <?php echo ( $value == 'autodrop_submenu' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Auto drop', 'wp-recruitment' ); ?></option>
                <option value="drop_full_width" <?php echo ( $value == 'drop_full_width' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Full width', 'wp-recruitment' ); ?></option>
            </select>
        </label>
    </p><?php
}
...
// widget_area.
add_action( 'wp_nav_menu_item_custom_fields', 'wp_recruitment_add_menu_widget_area_fields', 10, 4 );

function wp_recruitment_add_menu_widget_area_fields( $item_id, $item, $depth, $args ) {

    if(!$depth)
        return;

    $title = esc_html__('Widget Area', 'wp-recruitment'); $key = "menu-item-widget_area"; $value = $item->widget_area; $sidebars = $GLOBALS['wp_registered_sidebars'];

    ?><p class="description description-wide description_width_100 el_widget_area">
        <?php echo esc_html( $title ); ?><br />
        <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
            <select id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>">
                <option value="" <?php echo ( $value == '' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Select Widget Area', 'wp-recruitment' ); ?></option>
...
// item_group.
add_action( 'wp_nav_menu_item_custom_fields', 'wp_recruitment_add_menu_item_group_fields', 10, 4 );

function wp_recruitment_add_menu_item_group_fields( $item_id, $item, $depth, $args ) {
    $title = esc_html__('Group', 'wp-recruitment'); $key = "menu-item-group";
    ...
    <div>
        <label><?php esc_html_e('No', 'wp-recruitment'); ?>
        ...
        <label><?php esc_html_e('Yes', 'wp-recruitment'); ?>
...
/* one page */
add_action( 'wp_nav_menu_item_custom_fields', 'wp_recruitment_add_menu_is_onepage_fields', 10, 4 );

function wp_recruitment_add_menu_is_onepage_fields($item_id, $item, $depth, $args){

    $title = esc_html__('Is OnePage', 'wp-recruitment'); $key = "menu-item-is_onepage";
    ?>
    <p class="description description-wide description_width_100">
        <span><?php echo esc_html( $title ); ?></span><br />
    <div>
...
// hide_link.
add_action( 'wp_nav_menu_item_custom_fields', 'wp_recruitment_add_menu_hide_link_fields', 10, 4 );

function wp_recruitment_add_menu_hide_link_fields( $item_id, $item, $depth, $args ) {
    if(!$depth)
        return;

    $title = esc_html__('Hide title', 'wp-recruitment'); $key = "menu-item-hide_link"; $value = $item->hide_link;
    ...
    <p class="description description-wide description_width_100">
        ...
        <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
            <select id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>">
                <option value="0" <?php echo ( $value == '0' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'No', 'wp-recruitment' ); ?></option>
                <option value="1" <?php echo ( $value == '1' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Yes', 'wp-recruitment' ); ?></option>
            </select>
        </label>
    </p>
    <?php
...
// menu icon.
add_action( 'wp_nav_menu_item_custom_fields', 'wp_recruitment_add_menu_icon_fields', 10, 4 );

function wp_recruitment_add_menu_icon_fields( $item_id, $item, $depth, $args ) {

    $title = esc_html__('Menu Icon', 'wp-recruitment'); $key = "menu-item-menu_icon"; $value = $item->menu_icon;

    ?>
    <div id="<?php echo ''.$key . '-' . $item_id . '-popup'; ?>" data-item_id="<?php echo ''.$item_id;?>" class="menu_icon_wrap" style="display:none;">
        <?php
        $icons = apply_filters('theme/menu/icons', recruitment_font_awesome());
...
// menu background.
add_action( 'wp_nav_menu_item_custom_fields', 'wp_recruitment_add_menu_image_background_fields', 10, 4 );

function wp_recruitment_add_menu_image_background_fields( $item_id, $item, $depth, $args ) {



    $title = esc_html__('DropDown Background Image', 'wp-recruitment'); $key = "menu-item-bg_image"; $value = $item->bg_image;
    ...
    <p class="description description-wide obtheme_checkbox obtheme_mega_menu obtheme_mega_menu_d2">
        <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
            <span class='obtheme_long_desc'><?php echo esc_html( $title ); ?></span><br />
            <input type="text" value="<?php echo ''.$value; ?>" id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>" />
            <button id="browse-edit-<?php echo ''.$key . '-' . $item_id; ?>" class="set_custom_images button button-secondary submit-add-to-menu"><?php esc_html_e( 'Browse Image', 'wp-recruitment' ); ?></button>
            <a class="button btn_clear button-primary" href="javascript: void(0);">Clear</a>
        </label>
    </p>
    ...
    <?php $title = esc_html__('Background Color', 'wp-recruitment'); $key = "menu-item-bg_color"; $value = $item->bg_color; ?>
    ...
    <?php $title = esc_html__('Icon Color', 'wp-recruitment'); $key = "menu-item-icon_color"; $value = $item->icon_color; ?>
    ...
    <?php $title = esc_html__('Icon Font Size', 'wp-recruitment'); $key = "menu-item-icon_font_size"; $value = $item->icon_font_size; ?>
...
// menu background.
add_action( 'wp_nav_menu_item_custom_fields', 'wp_recruitment_add_menu_el_class_fields', 10, 4 );

function wp_recruitment_add_menu_el_class_fields( $item_id, $item, $depth, $args ) {

    $title = esc_html__('Extra Class', 'wp-recruitment'); $key = "menu-item-el_class"; $value = $item->el_class;
...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu  {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( ! empty( $item->_invalid ) ) {
            ...
            $title = sprintf( esc_html__( '%s (Invalid)', 'wp-recruitment'), $item->title );
        } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf( esc_html__('%s (Pending)', 'wp-recruitment'), $item->title );
        }
    ...
    <li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo implode(' ', $classes ); ?>">
        <dl class="menu-item-bar">
            <dt class="menu-item-handle">
                <span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo $submenu_text; ?>><?php esc_html_e( 'sub item' , 'wp-recruitment'); ?></span></span>
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'wp-recruitment'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                        echo esc_url( wp_nonce_url(
                            add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'wp-recruitment'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'wp-recruitment'); ?>" href="<?php
                    ...
                    ?>"><?php esc_html_e( 'Edit Menu Item', 'wp-recruitment' ); ?></a>
                </span>
            </dt>
        </dl>
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
            <?php if( 'custom' == $item->type ) : ?>
                <p class="field-url description description-wide">
                    <label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'URL', 'wp-recruitment' ); ?><br />
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Navigation Label', 'wp-recruitment' ); ?><br />
                    <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                </label>
            </p>
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Title Attribute', 'wp-recruitment' ); ?><br />
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                    ...
                    <?php esc_html_e( 'Open link in a new window/tab', 'wp-recruitment' ); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'CSS Classes (optional)', 'wp-recruitment' ); ?><br />
                    <input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                </label>
            </p>
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Link Relationship (XFN)', 'wp-recruitment' ); ?><br />
                    <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
                </label>
            </p>
            <p class="field-description description description-wide">
                <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Description', 'wp-recruitment' ); ?><br />
                    <textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
                    <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'wp-recruitment' ); ?></span>
                </label>
            </p>
            ...
            <p class="field-move hide-if-no-js description description-wide">
                <label>
                    <span><?php esc_html_e( 'Move', 'wp-recruitment' ); ?></span>
                    <a href="#" class="menus-move menus-move-up" data-dir="up"><?php esc_html_e( 'Up one', 'wp-recruitment' ); ?></a>
                    <a href="#" class="menus-move menus-move-down" data-dir="down"><?php esc_html_e( 'Down one', 'wp-recruitment' ); ?></a>
                    <a href="#" class="menus-move menus-move-left" data-dir="left"></a>
                    <a href="#" class="menus-move menus-move-right" data-dir="right"></a>
                    <a href="#" class="menus-move menus-move-top" data-dir="top"><?php esc_html_e( 'To the top', 'wp-recruitment' ); ?></a>
                </label>
            </p>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php printf( esc_html__('Original: %s', 'wp-recruitment' ), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                    </p>
                <?php endif; ?>
                ...
                ) ); ?>"><?php esc_html_e( 'Remove', 'wp-recruitment' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
                ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'wp-recruitment' ); ?></a>
            </div>
