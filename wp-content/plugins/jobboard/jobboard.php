...
define("JB_VERSION",'1.1.2.8');
define("JB_TEXT_DOMAIN",'jobboard');
...
//            require_once $this->plugin_directory . 'update/update-manager.php';
            ...
            require_once $this->plugin_directory . 'inc/class.jb-extension.php';
            ...
            $this->extension = new JobBoard_Extension();
            ...
            if (isset($screen->id)) {
                switch ($screen->id) {
                    case 'edit-jobboard-post-jobs':
                        ...
                        wp_enqueue_script('jobboard-edit-post', $this->plugin_directory_uri . 'assets/js/edit-post.js', array('jquery'), time(), true);
                        break;
        ...
        {
            load_plugin_textdomain(JB_TEXT_DOMAIN, false, basename( dirname( __FILE__ ) ) . '/languages');
        }
        ...
        {
            $action_links = array(
                'settings' => '<a href="' . admin_url('edit.php?post_type=jobboard-post-jobs&page=JobBoard') . '" title="' . esc_attr(esc_html__('JobBoard Settings', JB_TEXT_DOMAIN)) . '">' . esc_html__('Settings', JB_TEXT_DOMAIN) . '</a>',
            );
        ...
        {
            if ($plugin_file !== 'jobboard/jobboard.php') {
                return $plugin_meta;
            }

            $plugin_meta[] = '<a href="https://github.com/vianhtu/jobboard">' . esc_html__('GitHub', JB_TEXT_DOMAIN) . '</a>';
            $plugin_meta[] = '<a href="https://github.com/vianhtu/jobboard/issues/new" title="' . esc_html__('Report issues or New Features.', JB_TEXT_DOMAIN) . '">' . esc_html__('Issues', JB_TEXT_DOMAIN) . '</a>';
            $plugin_meta[] = '<a href="http://fsflex.com/support/job-board" title="' . esc_html__('Support forum.', JB_TEXT_DOMAIN) . '">' . esc_html__('Support', JB_TEXT_DOMAIN) . '</a>';
            $plugin_meta[] = '<a href="mailto:thienduongvangem.ntt@gmail.com" title="' . esc_html__('Send a email to Dev team.', JB_TEXT_DOMAIN) . '">' . esc_html__('Contact', JB_TEXT_DOMAIN) . '</a>';
