...
<div class="file type-<?php echo esc_attr($input); ?>">
    <?php if(!empty($value['id']) && $attachment = get_post($value['id'])): ?>
        <div class="file-thumb" title="<?php echo esc_attr(sprintf(esc_attr__('Uploaded : %s', JB_TEXT_DOMAIN), $attachment->post_date)); ?>">
        ...
        <div class="file-info">
            ...
            <span class="file-date"><?php echo sprintf(esc_html__('Uploaded %s', JB_TEXT_DOMAIN), jb_get_timeago(strtotime($attachment->post_date))); ?></span>
        </div>
    <?php elseif($input == 'image'): ?>
        <div class="file-thumb"><?php jb_the_placeholder_image('100x100'); ?></div>
        <div class="file-info">
            <span class="file-name"><?php esc_html_e('No image !', JB_TEXT_DOMAIN); ?></span>
        </div>
    <?php else:?>
        <div class="file-thumb"><i class="fa fa-file-o" aria-hidden="true"></i></div>
        <div class="file-info">
            <span class="file-name"><?php esc_html_e('No file !', JB_TEXT_DOMAIN); ?></span>
        </div>
    <?php endif;?>
</div>
<input id="<?php echo esc_attr($id); ?>" type="file" class="field-media" name="<?php echo esc_attr($name); ?>" data-type="<?php echo esc_attr($types); ?>" data-type-notice="<?php echo sprintf(esc_attr__('File format not supported, you can only upload the following "%s" file.', JB_TEXT_DOMAIN), $types); ?>" data-size="<?php echo esc_attr($size); ?>" data-size-notice="<?php echo sprintf(esc_attr__('You can not upload files larger than %sKb', JB_TEXT_DOMAIN), $size); ?>">
