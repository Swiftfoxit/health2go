...
//            add_filter( 'pre_set_site_transient_update_plugins', array( $this, 'update_plugins' ) );
        ...
        function plugin_message($plugin_data){
            ...
            echo '<a href="javascript:void(0)" class="verify-purchase-key">' . (empty($data['purchase_key']) ? esc_html__(' Verify purchase key', JB_TEXT_DOMAIN) : esc_html__(' Edit purchase key', JB_TEXT_DOMAIN)) . '</a>';
        }
        ...
        function plugin_row($plugin_file, $plugin_data){
            ...
            <tr class="plugin-verify verify-<?php echo esc_attr( $plugin_data['slug'] ); ?>" style="display: none;" data-slug="<?php echo esc_attr( $plugin_data['slug'] ); ?>" data-plugin="<?php echo esc_attr($plugin_file); ?>">
                <td colspan="3" class="plugin-update">
                    <input type="text" name="user_name" value="<?php echo esc_attr($data['user_name']); ?>" placeholder="<?php esc_html_e('Username', JB_TEXT_DOMAIN); ?>">
                    <input type="password" name="api_key" value="<?php echo esc_attr($data['api_key']); ?>" placeholder="<?php esc_html_e('Api Key', JB_TEXT_DOMAIN); ?>">
                    <input type="password" name="purchase_key" value="<?php echo esc_attr($data['purchase_key']); ?>" placeholder="<?php esc_html_e('Purchase Key', JB_TEXT_DOMAIN); ?>">
                    <input type="button" value="<?php esc_attr_e('Save', JB_TEXT_DOMAIN); ?>" class="button button-primary verify-save">
                </td>
            </tr>
            <?php
        }
