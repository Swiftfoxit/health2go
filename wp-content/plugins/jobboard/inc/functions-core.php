...
function jb_get_timeago($ptime)
{
    ...
    if ($estimate_time < 1) {
        return esc_html__('1 Second Ago', JB_TEXT_DOMAIN);
    }

    $condition = array(
        12 * 30 * 24 * 60 * 60 => esc_html__('year', JB_TEXT_DOMAIN),
        30 * 24 * 60 * 60 => esc_html__('month', JB_TEXT_DOMAIN),
        24 * 60 * 60 => esc_html__('day', JB_TEXT_DOMAIN),
        60 * 60 => esc_html__('hour', JB_TEXT_DOMAIN),
        60 => esc_html__('minute', JB_TEXT_DOMAIN),
        1 => esc_html__('second', JB_TEXT_DOMAIN)
    );
    ...
    foreach ($condition as $secs => $str) {
        ...
        if ($d >= 1) {
            ...
            return $r . ' ' . $str . ($r > 1 ? 's' : '') . ' ' . esc_html__('ago', JB_TEXT_DOMAIN);
        }
    }
}
...
function jb_error_args($error = true, $message = '')
{
    return array(
        'error' => $error,
        'message' => $message
...
function jb_sort_terms(Array &$terms, Array &$into, $parent = 0)
{
    ...
    if (count($terms) > 0 && $parent !== $old_parent) {
...
function jb_get_location_thum($id)
{
    if (!isset($id)) {
        return '';
    }
    $thumbnail = jb_get_term_meta($id, '_thumbnail');
...
function jb_get_term_meta($term_id, $key)
{
    if (!isset($term_id) || !isset($key)) {
        return false;
    }
...
function jb_get_locations($parent = '0', $count = 4, $hide_empty = false)
{
    if ($parent !== 'all') {
        $terms = jb_get_terms(array(
        ...
        $term_lv1 = jb_get_terms(array(
            'taxonomy' => 'jobboard-tax-locations',
            'hide_empty' => $hide_empty,
            'parent' => 0
        ));
        $terms = array();
        foreach ($term_lv1 as $key => $name){
            $term_lv2 = jb_get_terms(array(
                'taxonomy' => 'jobboard-tax-locations',
                'hide_empty' => $hide_empty,
                'parent' => $key
            ));
            $terms = array_merge($terms,array_keys($term_lv2));
        }
        return $terms;
    }
}
