...
function jb_account_get_media_url( $file = '' ) {
	...
	return apply_filters( 'jobboard_account_media_url', $upload_dir['baseurl'] . '/' . $file );
}
...
function jb_account_change_password( $user_id ) {
	...
	if ( strlen( $_POST['new-password'] ) < 8 ) {
		...
		jb_notice_add( esc_html__( 'Error : password must be greater than 8 characters.', JB_TEXT_DOMAIN ), 'error' );
	...
	if ( $_POST['new-password'] !== $_POST['confirm-password'] ) {
		...
		jb_notice_add( esc_html__( 'Error : new password not same confirm password.', JB_TEXT_DOMAIN ), 'error' );
	...
	jb_notice_add( esc_html__( 'Success : Password Changed !', JB_TEXT_DOMAIN ) );
}
...
function jb_account_get_type( $slug = false ) {
	...
	if ( is_jb_candidate( $jobboard_account->ID ) ) {
		return $slug ? 'candidate' : esc_html__( 'Candidate', JB_TEXT_DOMAIN );
	} elseif ( is_jb_employer( $jobboard_account->ID ) ) {
		return $slug ? 'employer' : esc_html__( 'Employer', JB_TEXT_DOMAIN );
	}

	return esc_html__( 'User', JB_TEXT_DOMAIN );
}
...
function jb_account_get_account_slug( $user_id = '' ) {
	global $jobboard_account;
	if ( ! $user_id && isset( $jobboard_account->ID ) ) {
		$user_id = $jobboard_account->ID;
	}
...
function jb_account_get_permalink( $user_id = '' ) {
	...
	if ( is_jb_employer( $user_id ) ) {
		$slug = jb_get_option( 'profile-employer-slug', 'employer' );
	} elseif ( is_jb_candidate( $user_id ) ) {
		$slug = jb_get_option( 'profile-candidate-slug', 'candidate' );
	}

	if ( get_option( 'permalink_structure' ) ) {
		return site_url( "/{$slug}/{$user->user_login}" );
	} else {
		return site_url( "/?account={$user->user_login}" );
	}
}
...
function jb_account_get_date( $format = 'M Y', $user_id = null ) {
	global $jobboard_account;
	if ( isset( $user_id ) ) {
		$account = get_userdata( $user_id );
		if ( $account !== false ) {
			$jobboard_account = $account;
		}
	}
