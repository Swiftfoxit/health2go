...
class JobBoard_FormHandler
{
    ...
    {
        /**
         * Set content to specialisms list page
         */
        if (!empty(jb_get_option('page-list-specialisms')) && is_page(jb_get_option('page-list-specialisms'))) {
            add_filter('the_content', function ($contents) {
                $contents = '[jobboard-alpha-beta-terms]';
                return $contents;
            });
        }
    ...
    function user_apply_job()
    {
        ...
        if (!empty($validate)) {
            ...
            jb_notice_add(esc_html__('Error : You need to enter all required fields.', JB_TEXT_DOMAIN), 'error');
            return false;
        }
        ...
        if (!$post) {
            jb_notice_add(esc_html__('Error : Job does not exists.', JB_TEXT_DOMAIN), 'error');
            return false;
        }

        if ($post->post_type != 'jobboard-post-jobs' || $post->post_status != 'publish') {
            jb_notice_add(esc_html__('Error : Job unpublished.', JB_TEXT_DOMAIN), 'error');
            return false;
        }
        ...
        if (!isset($apply['error'])) {
            ...
            jb_notice_add(sprintf(esc_html__("Successfully Applied for '%s'", JB_TEXT_DOMAIN), $post->post_title), 'success');

            do_action('jobboard_applied_job', $user_id, $post);
        } else {
            jb_notice_add(sprintf(esc_html__('Error : %s', JB_TEXT_DOMAIN), $apply['message']), 'error');
        }
    ...
    function user_edit_profile()
    {
        ...
        jb_notice_add(esc_html__('Successfully Your Profile Updated.', JB_TEXT_DOMAIN));
    ...
    function user_add_job()
    {
        ...
        if (is_wp_error($post_id = wp_insert_post($this->post))) {
            jb_notice_add(sprintf(esc_html__('Error : %s', JB_TEXT_DOMAIN), $post_id->get_error_message()), 'error');
            return;
        }
        ...
        jb_notice_add(sprintf(esc_html__("Successfully Added for '%s'", JB_TEXT_DOMAIN), get_the_title($post_id)));
    }
    ...
    function user_edit_job()
    {
        ...
        jb_notice_add(sprintf(esc_html__("Successfully Updated for '%s'", JB_TEXT_DOMAIN), get_the_title($post_id)));
    }
    ...
    function user_send_message()
    {
        if (empty($_POST['id']) || empty($_POST['contact-name']) || empty($_POST['contact-email']) || empty($_POST['contact-message'])) {
            jb_notice_add(esc_html__('You need to enter all required fields.', JB_TEXT_DOMAIN), 'error');
            return;
        }
        ...
        if (!$user) {
            jb_notice_add(esc_html__('User does not exist.', JB_TEXT_DOMAIN), 'error');
            return;
        }
        ...
        jb_notice_add(esc_html__("Successfully Message sent.", JB_TEXT_DOMAIN));
    }
