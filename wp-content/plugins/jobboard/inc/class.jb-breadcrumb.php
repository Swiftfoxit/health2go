...
class JobBoard_Breadcrumb {
	...
	private function add_crumbs_404() {
		$this->add_crumb( __( 'Error 404', JB_TEXT_DOMAIN ) );
	}
    ...
    private function add_crumbs_jb_location() {
        ...
        $this->prepend_job_page();
        $this->term_ancestors( $current_term->term_id, 'jobboard-tax-types' );
        $this->add_crumb( sprintf( __( 'Location: %s', JB_TEXT_DOMAIN ), $current_term->name ));
    }
	...
	private function add_crumbs_jb_tag() {
		...
		$this->prepend_job_page();
		$this->add_crumb( sprintf( __( 'Jobs tagged &ldquo;%s&rdquo;', JB_TEXT_DOMAIN ), $current_term->name ) );
	}
	...
	private function add_crumbs_jb_jobs() {
		...
		$_name = jb_page_id( 'jobs' ) ? get_the_title( jb_page_id( 'jobs' ) ) : esc_html__('Jobs', JB_TEXT_DOMAIN);
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( __( 'Posts tagged &ldquo;%s&rdquo;', JB_TEXT_DOMAIN ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_jb_employer_jobs() {
        ...
        $name       = jb_page_id( 'jobs' ) ? get_the_title( jb_page_id( 'jobs' ) ) : esc_html__('Jobs', JB_TEXT_DOMAIN);
	...
	private function add_crumbs_author() {
		...
		$userdata = get_userdata( $author );
		$this->add_crumb( sprintf( __( 'Author: %s', JB_TEXT_DOMAIN ), $userdata->display_name ) );
	}
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( sprintf( __( 'Search results for &ldquo;%s&rdquo;', JB_TEXT_DOMAIN ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( sprintf( __( 'Page %d', JB_TEXT_DOMAIN ), get_query_var( 'paged' ) ) );
		}
	}
}
