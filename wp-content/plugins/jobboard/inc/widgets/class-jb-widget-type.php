...
class JB_Widget_Type extends JB_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'A list of job types.', JB_TEXT_DOMAIN );
		$this->widget_id          = 'jobboard-widget-types';
		$this->widget_name        = esc_html__( 'JobBoard Job Types', JB_TEXT_DOMAIN );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Vacancy Type', JB_TEXT_DOMAIN ),
				'label' => esc_html__( 'Title', JB_TEXT_DOMAIN )
			),
			'orderby' => array(
				'type'  => 'select',
				'std'   => 'name',
				'label' => esc_html__( 'Order by', JB_TEXT_DOMAIN ),
				'options' => array(
					'order' => esc_html__( 'Category Order', JB_TEXT_DOMAIN ),
					'name'  => esc_html__( 'Name', JB_TEXT_DOMAIN ),
				)
			),
			'count' => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__( 'Show job counts', JB_TEXT_DOMAIN )
			),
            'color' => array(
                'type'  => 'checkbox',
                'std'   => 1,
                'label' => esc_html__( 'Show type color', JB_TEXT_DOMAIN )
            ),
			'hide_empty' => array(
				'type'  => 'checkbox',
				'std'   => 0,
	...
	public function widget( $args, $instance ) {
        ...
        $list_args['show_option_none']           = esc_html__('Type does not exists.', JB_TEXT_DOMAIN );
