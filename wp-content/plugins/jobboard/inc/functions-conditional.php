 ...
 *
 * @param int|string|object $user Optional. User ID, Object User. Default empty.
 * @return bool is candidate user.
 */
function is_jb_candidate($user = '') {
    if(empty($user)){
        $user = wp_get_current_user();
    } elseif (!is_object($user)){
        $user = get_user_by('id', $user);
    }
    return in_array('jobboard_role_candidate', jb_get_current_user_role($user->ID));
}
...
function is_jb_applied(){
    global $post;
    if(empty($post->app_status)) {
        $post->app_status = JB()->job->get_status($post->ID);
    }
    return  $post->app_status == 'applied' ? true : false;
}
