...
class CS_Recent_Post_Carousel extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'cs_recent_post_carousel', esc_html__('CMS Recent Post',"wp-recruitment"), array('description' => esc_html__('Recent Posts Widget.', "wp-recruitment"),)
        );
    }
    ...
    function widget($args, $instance) {
        ...
        $title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Recent Posts', "wp-recruitment" ) : $instance['title'], $instance, $this->id_base);
                                ...
                                <div class="cms-recent-media">  
                                   <div class="image">
                                        <?php if(has_post_thumbnail($post->ID)) {
                                            $thumbnail = wp_get_attachment_url( get_post_thumbnail_id($post->ID, 'recruitment_350X350') );
                                            $class = 'has-thumbnail';
                                        } else {
    ...
    function form($instance) {
        ...
        ?>
        <p>
            <label for="<?php echo ''.$this->get_field_id('title'); ?>"><?php esc_html_e( 'Title:', "wp-recruitment" ); ?></label>
        ...
        <p>
            <label for="<?php echo ''.$this->get_field_id('number'); ?>"><?php esc_html_e( 'Number of post to show:', "wp-recruitment" ); ?></label>
